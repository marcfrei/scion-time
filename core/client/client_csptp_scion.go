...
func readCSPTPReplySCION(ctx context.Context, log *slog.Logger,
	...
	for numRetries := 0; ; numRetries++ {
		...
		err = decodeCSPTPReply(&reply, udpLayer.Payload, sequenceID, flashPTP)
		if err != nil {
			if numRetries != maxNumRetries && deadlineSet && timebase.Now().Before(deadline) {
		...
		srcPort := uint16(csptp.EventPortSCION)
		if reply.msg.MessageType() == csptp.MessageTypeFollowUp {
			srcPort = csptp.GeneralPortSCION
		}
		if udpLayer.SrcPort != srcPort {
...
func (c *CSPTPClientSCION) MeasureClockOffset(ctx context.Context, localAddr, remoteAddr udp.UDPAddr, path snet.Path) (
	...
	for !replies.complete() {
		...
		err = readCSPTPReplySCION(ctx, c.Log,
			conn, localAddr, remoteAddr, deadline, deadlineSet,
