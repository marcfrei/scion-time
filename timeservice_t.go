...
func runT() {
	if len(os.Args) > 2 && os.Args[2] == "ntlmcs" {
		runTNTLMCS()
	} else {
		var (
			laddr, raddr string
...
func runTNTLMCS() {
	...
	flags := flag.NewFlagSet("ntlmcs", flag.ExitOnError)
	flags.StringVar(&config, "config", "",
		"NTL MCS config: phc_dev,cts_dev,target_index[,initial_drift_ppb[,poll_interval_ms]]")

	err := flags.Parse(os.Args[3:])
	if err != nil || flags.NArg() != 0 || config == "" {
		panic("failed to parse ntlmcs arguments")
	}

	initLogger(logLevelVerbose)
	log := slog.Default()

	service.StartNTLMCSSync(log, config)
	select {}
}
